...
export class TokenBucket {
  ...
  private _scriptSha: string;
  private _scriptLoaded: boolean;

  constructor({ capacity = 10, refillRate = 1, refillInterval = 1.0 } = {}) {
  ...
  // ✅ lazy Redis getter — initializes on first use
  private async getRedis(): Promise<Redis> {
    if (!this.redis) {
      this.redis = await redisConnection.getRedisOrConnect();
    }
    return this.redis;
  }
  ...
  async _ensureScriptLoaded() {
    ...
    if (!this._scriptLoaded) {
      try {
        this._scriptSha = (await this.redis.script(
          "LOAD",
          TOKEN_BUCKET_SCRIPT,
        )) as string;
  ...
  async allow(key: string) {
    ...
    try {
      ...
      if (err.message?.includes("NOSCRIPT")) {
        // fallback to EVAL with actual script
    ...
    const allowed = Boolean((result as any)[0]);
    const remaining = Number((result as any)[1]);
